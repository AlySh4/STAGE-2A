...
class UdpMessagesInterface(threading.Thread):
    def __init__(self, callback, verbose=False,
        ...
        try:
            ...
            logger.error("Error: unable to open socket on ports '%d' (up) and '%d' (down)" % (
                    ...
                    for c in msg:
                        ...
                        if self.trans.parse_byte(c):
                            try:
                                ...
                                if self.verbose:
                                    logger.info("New incoming message '%s' from %i (%i, %s) to %i" % (
                                        msg.name, sender_id, component_id, address, receiver_id))
...
def test():
    ...
    parser.add_argument("-c", "--class", help="message class of incoming messages", dest='msg_class',
                        default='telemetry')
    ...
    parser.add_argument("-dp", "--downlink_port", help="downlink port", dest='downlink', default=DOWNLINK_PORT,
                        type=int)
