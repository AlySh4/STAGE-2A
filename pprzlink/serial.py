                    ...
                    if self.trans.parse_byte(c):
                        try:
                            ...
                            if self.verbose:  # See the note on the same line in v1.0
                                logger.info("New incoming message '%s' from %i (%i) to %i" % (
                                    msg.name, sender_id, component_id, receiver_id))
...
def test():
    ...
    serial_interface = SerialMessagesInterface(lambda s, m: print("new message from %i: %s" % (s, m)), device=args.dev,
                                               baudrate=args.baud, msg_class=args.msg_class, interface_id=args.id,
                                               verbose=True)
